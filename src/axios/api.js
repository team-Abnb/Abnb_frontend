...
// 좋아요 조회
const getLike = async (roomId) => {
    const response = await instance.get(`/api/wishlist`);
    return response.data;
};

// 좋아요 등록
const postLike = async (likeWishlist) => {
    const response = await instance.post(`/api/wishlist`, likeWishlist);
    console.log("좋아용", response);
    const token = localStorage.getItem("token");
    localStorage.getItem("token", token);
    return response.data;
};

export { addUsers, login, getRoomPosts, getRoomsDetail, getComments, createRoom, getLike, postLike };
